...
function paint(item: ISimelDescription, img: string) {
    ...
    return (
        <div className="contentPaint">
            <div>
                <ReactSVG className={"imk" + procent} src={img} />
            </div>
            <p className="contentPaintText">{item.title}</p>
            <p
                className="contentPaintPercent"
                style={{ color: item.colorProcent }}
            >
                {item.procent + "%"}
            </p>
        </div>
